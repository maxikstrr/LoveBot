...
function renderError(name, ...parameters) {
  ...
  return new FfmpegError(code, (0, import_node_util.format)(template, ...parameters));
}
...
function inferFfprobePath(ffmpegPath, fallback) {
  ...
  if (!(0, import_node_path.isAbsolute)(ffmpegPath)) return fallback;
  const suffix = process.platform === "win32" ? ".exe" : "";
  return (0, import_node_path.join)(ffmpegPath, "..", `ffprobe${suffix}`);
}
...
function terminateProcessTree(child, signal, platform = process.platform, spawnProcess = import_node_child_process.spawn, killGroup = (pid, groupSignal) => process.kill(pid, groupSignal)) {
...
var Video = class extends import_node_events.EventEmitter {
  ...
  resolveWatermark(watermarkPath, settings) {
    if (!(0, import_node_fs.existsSync)(watermarkPath)) throw renderError("invalid_watermark", watermarkPath);
  ...
  fnExtractSoundToMP3(destination, callback) {
    const extension = (0, import_node_path2.extname)(destination).toLowerCase() === ".mp3" ? destination : (0, import_node_path2.join)((0, import_node_path2.dirname)(destination), `${(0, import_node_path2.basename)(destination, (0, import_node_path2.extname)(destination))}.mp3`);
  ...
  fnAddWatermark(watermarkPath, ...args) {
    ...
    const resolvedDestination = destination ?? (0, import_node_path2.join)(
      (0, import_node_path2.dirname)(this.file_path),
      `${(0, import_node_path2.basename)(this.file_path, (0, import_node_path2.extname)(this.file_path))}_watermark_${(0, import_node_path2.basename)(
        watermarkPath,
        (0, import_node_path2.extname)(watermarkPath)
      )}${(0, import_node_path2.extname)(this.file_path)}`
    );
  ...
  async extractFrames(destinationFolder, settings, context) {
    ...
    try {
      await (0, import_promises.mkdir)(destinationFolder, { recursive: true });
    } catch (cause) {
    ...
    let fileName = settings.file_name ?? (0, import_node_path2.basename)(this.file_path, (0, import_node_path2.extname)(this.file_path));
    ...
    fileName = `${(0, import_node_path2.basename)(fileName, (0, import_node_path2.extname)(fileName))}_%d.jpg`;
    const outputPattern = (0, import_node_path2.join)(destinationFolder, fileName);
    ...
    }).map(([entry]) => entry).sort((left, right) => left.localeCompare(right, void 0, { numeric: true })).map((entry) => (0, import_node_path2.join)(destinationFolder, entry));
  }
  ...
  createOperationContext(kind, destination) {
    return Object.freeze({ operationId: (0, import_node_crypto.randomUUID)(), kind, destination });
  }
};
async function frameFingerprints(directory, matcher) {
  const entries = (await (0, import_promises.readdir)(directory)).filter((entry) => matcher.test(entry));
  const fingerprints = /* @__PURE__ */ new Map();
  await Promise.all(
    entries.map(async (entry) => {
      try {
        const details = await (0, import_promises.stat)((0, import_node_path2.join)(directory, entry));
...
function validateInput(input) {
  ...
  if (!isRemoteInput(input) && !(0, import_node_fs2.existsSync)(input)) throw renderError("fileinput_not_exist", input);
}
